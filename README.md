...
Reference implementation of a two-tier safe execution model for LLM agents written in Go.
Tier 1: autonomous exploration → Deterministic validation boundary (MCP server) → Tier 2: bounded execution (durable Inngest workflows).
`Go` `Inngest` `Omnigent` `Docker`
...
Go + Next.js full-stack monolith with vertical feature slices and pluggable Postgres/SQLite · Redis/MemCache · RabbitMQ/EventBus adapters. 556 commits, CI + E2E.
